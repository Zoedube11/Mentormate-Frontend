  ...
  const sendMessage = async () => {
    ...
    try {
      const resp = await fetch(RAG_API, {
        ...
        body: JSON.stringify({ query: txt, top_k: 5, collection_name: "Mining_docs" })
      });
      ...
      setMessages(prev => [
        ...prev,
        {
          id: `a-${Date.now()}`,
          type: "assistant",
          content: data.answer || "No answer right now.",
          timestamp: new Date().toLocaleTimeString()
        }
      ]);
    } catch (err) {
        ...
        <div className="max-w-4xl w-full bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col">
          ...
          <div className="p-4 border-b flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">Mining & metallurgical specialist</div>
            </div>
          ...
          <div className="flex-1 p-6 space-y-4 overflow-y-auto max-h-[70vh]">
            {messages.map((m) => (
              <div key={m.id} className={`flex items-end ${m.type === "user" ? "justify-end" : "justify-start"}`}>
                {m.type === "assistant" && (
                  <img src={SisNandiImg} alt="avatar" className="w-8 h-8 rounded-full mr-2" />
                )}
                <div className={`${m.type === "user" ? "bg-gray-200 text-gray-900" : "bg-purple-50 text-gray-900"} rounded-2xl px-4 py-2 max-w-[70%] shadow-md animate-fade-in`}>
          ...
          <div className="p-4 border-t flex gap-2">
            <textarea
              ...
              onKeyDown={handleKey}
              placeholder="Ask about ore processing, circuits, or safety..."
              className="flex-1 border rounded-full px-4 py-2 resize-none focus:ring-2 focus:ring-purple-400"
              rows={1}
            />
            <button onClick={sendMessage} className="bg-purple-600 text-white px-6 py-2 rounded-full hover:bg-purple-500 transition-colors">
              Send
            </button>
          </div>
        </div>
