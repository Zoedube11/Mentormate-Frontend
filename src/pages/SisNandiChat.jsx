...
export default function SisNandiChat() {
  ...
  const [currentMentor, setCurrentMentor] = useState("Sis Nandi");
    ...
    <div className="flex min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ${isSidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        ...
        <header className="flex items-center gap-4 px-6 py-4 border-b bg-white sticky top-0 z-10">
          ...
          <img src={SisNandiImg} alt="Sis Nandi" className="w-12 h-12 rounded-full object-cover" />
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-semibold truncate">Sis Nandi</h1>
            <p className="text-xs text-gray-500 truncate">Mining & Metallurgical Specialist</p>
          </div>
          <button
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.type === "user" ? "justify-end" : "justify-start"} items-end gap-3`}>
              {m.type === "assistant" && (
                <img src={SisNandiImg} alt="avatar" className="w-24 h-24 rounded-full shadow-sm" />
              )}
              <div
                className={`${
                  m.type === "user" 
                    ...
                    : m.id === "welcome" 
                      ? "bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 text-gray-800" 
                      : "bg-[rgb(245,206,206)] text-gray-900"
                ...
                {m.id === "welcome" ? (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-semibold text-lg text-purple-900">Welcome Message</h3>
                    </div>
          ...
          {isTyping && (
            <div className="flex items-center gap-2 text-gray-500 italic">
              <img src={SisNandiImg} alt="avatar" className="w-6 h-6 rounded-full" />
              <div className="flex gap-1">
                <span className="dot animate-bounce bg-purple-400"></span>
                <span className="dot animate-bounce delay-200 bg-purple-400"></span>
        ...
        <footer className="border-t p-4 bg-white flex gap-3">
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder="Ask Sis Nandi about ore processing, metallurgical circuits, or safety..."
            className="flex-1 border rounded-full px-4 py-2 resize-none focus:ring-2 focus:ring-purple-400 bg-gray-50 text-sm lg:text-base"
            rows={1}
          />
          <button
            onClick={sendMessage}
            disabled={!inputValue.trim() || isTyping}
            className="bg-[rgb(172,97,97)] text-white px-6 py-2 rounded-full hover:bg-[rgb(245,206,206)] disabled:opacity-50"
          >
            Send
          </button>
        </footer>
