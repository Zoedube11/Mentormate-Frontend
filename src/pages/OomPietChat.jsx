...
// export default function OomPietChat() {
//   const [messages, setMessages] = useState([
//     {
//       id: "welcome",
//       type: "assistant",
//       content: "Hallo daar! Ek's Piet, a seasoned geotechnical engineer with deep roots in tailings, geology, and soil mechanics. I've been shaping South Africa's mining and infrastructure landscape since the first gold rush. Ask away!",
...
//         body: JSON.stringify({ query: txt, top_k: 5, collection_name: "Tailings_engineer_docs" })
//       });
//       const data = await resp.json();
//       const answerMsg = {
//         id: `a-${Date.now()}`,
//         type: "assistant",
//         content: data.answer || "Sorry, no answer available at the moment.",
//         timestamp: new Date().toLocaleTimeString()
//       };
...
//             <img src={OomPietImg} alt="Oom Piet" className="w-12 h-12 rounded-full object-cover" />
//             <div>
//               <h2 className="text-lg font-semibold">Oom Piet</h2>
//               <div className="text-sm text-gray-500">Tailings & geotechnical specialist</div>
//             </div>
//             <div className="ml-auto text-sm text-gray-400 font-bold">OP</div>
//           </div>

//           {/* Messages */}
//           <div className="flex-1 p-6 space-y-4 overflow-y-auto max-h-[70vh]">
//             {messages.map((m) => (
//               <div key={m.id} className={`flex items-end ${m.type === "user" ? "justify-end" : "justify-start"}`}>
//                 {m.type === "assistant" && (
//                   <img src={OomPietImg} alt="avatar" className="w-8 h-8 rounded-full mr-2" />
//                 )}
//                 <div className={`${m.type === "user" ? "bg-gray-200 text-gray-900" : "bg-orange-50 text-gray-900"} rounded-2xl px-4 py-2 max-w-[70%] shadow-md animate-fade-in`}>
...
//               onKeyDown={handleKey}
//               placeholder="Ask about tailings, soil mechanics, or geotechnical projects..."
//               className="flex-1 border rounded-full px-4 py-2 resize-none focus:ring-2 focus:ring-orange-400"
//               rows={1}
//             />
//             <button onClick={sendMessage} className="bg-orange-600 text-white px-6 py-2 rounded-full hover:bg-orange-500 transition-colors">
//               Send
//             </button>
//           </div>
//         </div>
...
//         .dot { width: 6px; height: 6px; background-color: #f97316; border-radius: 50%; display: inline-block; }
...
export default function OomPietChat() {
  ...
  const [isTyping, setIsTyping] = useState(false);
  const messagesEndRef = useRef(null);
  const [currentMentor, setCurrentMentor] = useState("Oom Piet");
  const [currentSessionId, setCurrentSessionId] = useState(`session-${Date.now()}`);
  const [currentConversationId, setCurrentConversationId] = useState(null);
  const collectionName = "Tailings_engineer_docs";
      ...
      <div className={`flex flex-col flex-1 transition-all duration-300 ${isSidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        ...
        <header className="flex items-center gap-4 px-6 py-4 border-b bg-white sticky top-0 z-10">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-semibold truncate">Oom Piet</h1>
            <p className="text-xs text-gray-500 truncate">Tailings & Environmental Specialist</p>
          </div>
          <button
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.type === "user" ? "justify-end" : "justify-start"} items-end gap-3`}>
              {m.type === "assistant" && (
                <img src={OomPietImg} alt="avatar" className="w-24 h-24 rounded-full shadow-sm" />
              )}
              <div
                className={`${
                  m.type === "user" 
                    ? "bg-gray-200" 
                    : m.id === "welcome" 
                      ? "bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 text-gray-800" 
                      : "bg-orange-50"
                } text-gray-900 px-5 py-4 rounded-2xl max-w-4xl shadow-lg ${
                ...
                {m.id === "welcome" ? (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-semibold text-lg text-orange-900">Welcome Message</h3>
                    </div>
          ...
          {isTyping && (
            <div className="flex items-center gap-2 text-gray-500 italic">
              <img src={OomPietImg} alt="avatar" className="w-6 h-6 rounded-full" />
              <div className="flex gap-1">
                <span className="dot animate-bounce bg-green-400"></span>
                <span className="dot animate-bounce delay-200 bg-green-400"></span>
                <span className="dot animate-bounce delay-400 bg-green-400"></span>
              </div>
              <span>Oom Piet is typing...</span>
            </div>
          )}
        ...
        <footer className="border-t p-4 bg-white flex gap-3">
          <textarea
            ...
            onChange={(e) => setInputValue(e.target.value)}
            onKeyDown={handleKey}
            placeholder="Ask Oom Piet about dam safety, soil mechanics, or environmental impact..."
            className="flex-1 border rounded-full px-4 py-2 resize-none focus:ring-2 focus:ring-green-400 bg-gray-50 text-sm lg:text-base"
            rows={1}
          />
          <button
            onClick={sendMessage}
            disabled={!inputValue.trim() || isTyping}
            className="bg-orange-400 text-white px-6 py-2 rounded-full hover:bg-orange-500 disabled:opacity-50"
          >
            Send
          </button>
        </footer>
